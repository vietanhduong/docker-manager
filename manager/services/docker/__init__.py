...
class DockerManager(object):
    ...
    def get_containers(self, container_id: str = None):
        with requests_unixsocket.Session() as session:
            response = session.get(f"{self.__BASE_URL}/containers/json?all=1")
            ret = []
    ...
    def container_inspect(self, container_id: str):
        with requests_unixsocket.Session() as session:
            resp = session.get(f'{self.__BASE_URL}/containers/{container_id}/json')
    ...
    def container_logs(self, container_id: str):
        ...
        with requests_unixsocket.Session() as session:
            with session.get(f"{self.__BASE_URL}/containers/{container_id}/logs?stderr=1&stdout=1&tail=all&follow=1",
    ...
    def events(self):
        ...
        """
        with requests_unixsocket.Session() as session:
            with session.get(f"{self.__BASE_URL}/events", stream=True) as response:
                for line in response.iter_lines():
                    yield line + b"\n"
    ...
    def __action_with_container(self, action, container_id):
        with requests_unixsocket.Session() as session:
            self.__check_container_existed(container_id)
            response = session.post(f"{self.__BASE_URL}/containers/{container_id}/{action}?t=1")
